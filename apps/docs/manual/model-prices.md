...
当已同步模型目录明确声明输入或输出模态时，模型价格页会展示例如“输入：text、image；输出：text”的**模型能力**。这仅表示目录声明的支持范围，不表示某次请求实际携带了图片或其他媒体。

当前 CPA/CLIProxyAPI 用量队列未提供按图片、音频或视频拆分的 Token 计数，因此 CPAMP 不会把模型能力、请求内容或缺失字段推断为 `0` 或任何估算值；总输入 Token 和成本仍遵循现有用量字段及价格规则。只有上游提供稳定、明确的模态 Token 契约后，才会显示单独的多模态 Token 指标或费率。
