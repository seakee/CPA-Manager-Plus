...
部分统一计费账号在已用百分比为零（或舍入为零）时会省略 `creditUsagePercent`。
只有设置接口明确确认 SuperGrok / SuperGrok Heavy，且同一凭证的补充 gRPC 账单响应完整、成功、
处于当前周账期并与 REST 账期一致时，面板和服务端巡检才将其识别为隐含的 0% 已用，页面显示 100% 剩余。
这不代表没有消耗过 token，也不能换算成剩余 token 数。

仅有 REST 周期信息、Free / 未确认套餐、过期或不完整响应仍保持未知（`-`）。
补充查询只读、通过现有 CPA 管理接口转发，最多额外等待 6 秒；失败不改变原有账单健康判定或触发账号操作。
