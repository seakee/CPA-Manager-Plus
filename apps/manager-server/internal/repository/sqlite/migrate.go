...
func Migrate(db *sql.DB) error {
	...
	statements := []string{
		...
		`create table if not exists gateway_quota_decision_events_v1 (
			decision_id text not null primary key check(length(decision_id) = 32 and decision_id not glob '*[^0-9a-f]*'),
			...
			api_key_id text not null check(length(api_key_id) = 32 and api_key_id not glob '*[^0-9a-f]*'),
			policy_id text not null check(length(policy_id) = 32 and policy_id not glob '*[^0-9a-f]*'),
			...
			enforcement text not null check(enforcement = 'observed'),
			action text not null check(action = 'notify'),
			outcome text not null check(outcome in ('within_limit', 'notify_required', 'indeterminate')),
