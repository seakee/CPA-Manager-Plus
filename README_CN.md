...
- **Codex 账号巡检**：批量探测 Codex 认证池并给出清理建议，可选 5 小时短窗口冷却策略；默认行为保持不变
- **日志**：增量读取和筛选文件日志
...
Codex 巡检默认保持既有行为：当凭证达到 5 小时额度阈值，但周/月长期额度仍可用时，账号会保留或重新启用。

对于较大的 Codex 账号池，可以把 **5 小时额度达到阈值时的处理方式** 改为 **短窗口冷却**。开启后，5 小时短窗口达到阈值但长期额度仍可用的账号会被暂时禁用，而不是删除；后续巡检发现短窗口恢复后可以再自动启用。该模式用于减少代理路由反复尝试短窗口耗尽账号，建议配合服务端定时 Codex 巡检使用，巡检间隔约 60 分钟，自动处理策略选择禁用。
