...
func TestUsageTokenAccountingMigrationWorkerRunsBatchesBeforeCompletion(t *testing.T) {
	...
	for _, hash := range []string{"legacy-1", "legacy-2"} {
		...
		) values (?, 1, '1', 'openai', 'gpt-5', 100, 10, 110, 1)`, hash); err != nil {
	...
	if _, err := rawDB.Exec(`update usage_data_migrations set
		...
		where name = 'usage_token_accounting_v2'`); err != nil {
		t.Fatalf("reset migration state: %v", err)
	}

	completed := make(chan struct{}, 1)
	w := NewUsageTokenAccountingMigrationWorker(st, func() { completed <- struct{}{} })
	w.batchSize = 1
	...
	state, err := st.UsageTokenAccountingMigrationState(context.Background())
	if err != nil {
	...
	if err := rawDB.QueryRow(`select count(*) from usage_events
		where normalized_total_output_tokens is null or trim(coalesce(accounting_quality, '')) = ''`).Scan(&remaining); err != nil {
...
func TestUsageTokenAccountingMigrationWorkerCompletesOnce(t *testing.T) {
	...
	completed := make(chan struct{}, 1)
	var calls int32
	w := NewUsageTokenAccountingMigrationWorker(db, func() {
